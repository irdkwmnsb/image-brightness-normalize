...
#pragma omp parallel for schedule(auto) collapse(2) default(none) shared(ys, img) reduction(min:min_y) reduction(max:max_y)
...
#pragma omp parallel for schedule(auto) collapse(2) default(none) shared(ys, max_y, min_y, img, new_max, new_min)
    for (int x = 0; x < img->width; x++) {
        for (int y = 0; y < img->height; y++) {
            float r = image_r(img, x, y), g = image_g(img, x, y), b = image_b(img, x, y);
